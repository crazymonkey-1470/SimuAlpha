...
| `simulate_strategy`  | `POST /v1/tools/simulate-strategy` | `simulate_strategy`  | `simulation_results` (hash-keyed) + freqtrade library-mode |

Auxiliary endpoints:
- `GET /v1/jobs/{id}` — poll any async job (backtest **or** simulate **or** chart-render batch).
...
- `supabase/migration_quant_simulate.sql` — `simulation_results` (hash-keyed cache). Same RLS posture.
...
- **Stage 4 (live):** `simulate_strategy` tool — StrategySpec DSL (entry + 5-tranche DCA + exit legs + stop + position sizing), freqtrade library-mode adapter, cross-tool annotated trade charts.
...
`docs/patterns/`, custom-expression DSL in `docs/custom-expression-dsl.md`,
StrategySpec DSL in `docs/strategy-dsl.md`, and a top-down tool map
...
- `requirements.txt` — core service (get_price_history,
  get_fundamentals, render_tli_chart, backtest_pattern).
- `requirements-stage4.txt` — simulate_strategy extras (freqtrade +
  pandas-ta from git). Install only on the Railway service(s) that
  need it:

  ```bash
  pip install -r requirements.txt -r requirements-stage4.txt
  ```

The core service is deliberately installable without Stage-4 extras
so freqtrade's heavier transitive deps don't block the other four
tools.
