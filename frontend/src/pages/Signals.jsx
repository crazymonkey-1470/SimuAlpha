...
const IntelligenceFeed = lazy(() => import('./IntelligenceFeed'));
const ConsensusLeaderboard = lazy(() => import('./ConsensusLeaderboard'));
    ...
    <div style={{ paddingTop: '40px' }}>
      <div style={{ marginBottom: '32px' }}>
        ...
        <p style={{
          ...
          Every entry, exit, intelligence, and consensus signal the system tracks.
        </p>
      ...
      {tab === 'entry' && <EntrySignals navigate={navigate} />}
      {tab === 'exit' && <ExitSignals navigate={navigate} />}
      {tab === 'intelligence' && (
      ...
      <div style={{
        ...
        padding: '20px 0', borderTop: '1px solid var(--border)', marginTop: '24px'
      }}>
...
function EntrySignals({ navigate }) {
...
function ExitSignals({ navigate }) {
  const { data: exitSignals, loading } = useExitSignals();

  if (loading) return <LoadingSpinner />;
  if (exitSignals.length === 0) {
    return (
      <EmptyState
        message="No exit signals yet"
        sub="Exit signals fire when wave targets are hit. They will appear after wave analysis detects target zones."
      />
    );
  }
