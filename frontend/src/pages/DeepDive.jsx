...
function ClaudeInterpretation({ interpretation }) {
  if (!interpretation || interpretation.error) return null;

  const CONVICTION_COLORS = { HIGH: '#00ff88', MEDIUM: '#f5a623', LOW: '#888', UNKNOWN: '#888' };
  const color = CONVICTION_COLORS[interpretation.conviction] || '#888';

  const oneLiner = interpretation.one_liner?.length > 80
    ? interpretation.one_liner.slice(0, 77) + '...'
    : interpretation.one_liner;

  return (
    <div className="bg-bg-card border border-border p-5 mt-4" style={{ borderLeftWidth: 3, borderLeftColor: color }}>
      <div className="flex items-center justify-between mb-3">
        <span className="font-heading text-[10px] text-text-secondary tracking-widest uppercase">AI Analysis</span>
        <span className="font-mono text-[10px] px-2 py-0.5 border" style={{ color, borderColor: color }}>
          {interpretation.conviction} CONVICTION
        </span>
      </div>

      <p className="font-heading text-base text-text-primary mb-3 italic leading-relaxed">"{oneLiner}"</p>
      <p className="font-mono text-[12px] text-text-secondary leading-relaxed mb-4">{interpretation.summary}</p>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-3">
        {[
          { l: 'Entry Guidance', v: interpretation.entry_guidance },
          { l: 'Confirmation', v: interpretation.confirmation_signals },
          { l: 'Invalidation', v: interpretation.invalidation },
          { l: 'Risk Factors', v: interpretation.risk_factors },
        ].map(({ l, v }) => (
          <div key={l} className="bg-bg border border-border/50 p-3">
            <div className="font-mono text-[9px] text-text-secondary uppercase tracking-wider mb-1">{l}</div>
            <div className="font-mono text-[11px] text-text-primary leading-relaxed">{v}</div>
          </div>
        ))}
      </div>

      <div className="flex items-center justify-between bg-bg border border-border/50 px-3 py-2">
        <span className="font-mono text-[9px] text-text-secondary uppercase tracking-wider">TLI Alignment</span>
        <span className="font-mono text-[11px] text-text-primary">{interpretation.tli_alignment}</span>
      </div>

      <div className="mt-2 font-mono text-[9px] text-text-dim">
        Generated by {interpretation.model_used} · {new Date(interpretation.interpreted_at).toLocaleDateString()}
      </div>
    </div>
  );
}
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      {waveCounts.length > 0 && (
        <div className="mb-6">
          ...
          <ClaudeInterpretation interpretation={waveCounts[0]?.claude_interpretation} />
        </div>
      )}
