...
/**
 * Deep-score a single ticker through the full pipeline.
 * Fetches fresh data from the scraper, runs the scorer, and upserts into screener_results.
 */
async function deepScoreSingle(ticker) {
  ticker = ticker.toUpperCase();
  console.log(`[deepScoreSingle] Scoring ${ticker} (scorer: ${SCORING_VERSION})...`);
  ...
  if (!fund || fund.currentPrice == null) {
    return { error: 'No fundamental data returned from scraper' };
  }
  ...
  const { data: prev } = await supabase
    .from('screener_results')
    .select('total_score, signal, current_price, price_200wma, price_200mma, sector')
    .eq('ticker', ticker)
    .maybeSingle();

  const previousScore = prev?.total_score ?? null;
  const previousSignal = prev?.signal ?? null;
  const sector = fund.sector || prev?.sector || null;

  // Institutional + macro + SAIN
  let institutionalData = null;
  let macroContext = null;
  let sainConsensus = null;
  ...
  // Sector avg PE
  let sectorAvgPE = null;
  if (sector) {
    const { data: sectorResults } = await supabase
      .from('screener_results')
      .select('pe_ratio')
      .eq('sector', sector)
      .not('pe_ratio', 'is', null);
    if (sectorResults?.length > 0) {
      const valid = sectorResults.filter(r => r.pe_ratio > 0 && r.pe_ratio < 500);
      if (valid.length > 0) {
        sectorAvgPE = Math.round(valid.reduce((a, b) => a + b.pe_ratio, 0) / valid.length * 10) / 10;
      }
    }
  }
  ...
  if (SCORING_VERSION === 'v3') {
    ...
    scores = computeTLIScoreV3({
      ...
      grossMarginCurrent: fund.grossMarginCurrent,
      grossMarginPriorYear: fund.grossMarginPriorYear ?? null,
      ttmEBITDA: fund.ebitda, totalDebt: fund.totalDebt,
      debtToEquity: fund.debtToEquity,
      epsGrowthQoQ: fund.epsGrowthQoQ ?? null,
      epsGrowthPriorQoQ: fund.epsGrowthPriorQoQ ?? null,
      epsGrowthYoY: fund.epsGrowthYoY ?? null,
      sharesOutstandingChange: fund.sharesOutstandingChange,
      insiderNetBuying: fund.insiderNetBuying ?? null,
      peRatio: fund.peRatio, forwardPE: enriched.forwardPE || fund.forwardPE,
      freeCashFlow: fund.freeCashFlow, fcfMargin: fund.fcfMargin,
      fcfGrowthYoY: fund.fcfGrowthYoY, marketCap: fund.marketCap,
      operatingMargin: fund.operatingMargin, netIncome: fund.netIncome,
      dividendYield, cashAndEquivalents: fund.cashAndEquivalents,
      sector, moatTier: fund.moatTier ?? null,
      previousLow: week52Low, week52High: fund.week52High, week52Low,
      deathCrossActive: deathCross,
      sma50SlopeNegative: ma50d != null && price200WMA != null && ma50d < price200WMA,
      ...
      returnTo200wmaPct: (price200WMA != null && currentPrice > 0 && price200WMA > currentPrice)
        ? ((price200WMA - currentPrice) / currentPrice) * 100 : null,
      epsDiluted: fund.epsDiluted,
    }, waveAnalysis, macroContext, institutionalData, sainConsensus);
  } else {
    scores = runScorer({
      currentPrice, week52High: fund.week52High, week52Low,
      price200WMA, price200MMA,
      revenueGrowthPct: fund.revenueGrowthPct, psRatio: fund.psRatio, peRatio: fund.peRatio,
      ma50d, goldenCross, deathCross, hhHlPattern, previousScore,
      revenueGrowth3YrAvg: fund.revenueGrowth3YrAvg,
      fcfMargin: fund.fcfMargin, fcfGrowthYoY: fund.fcfGrowthYoY,
      cashAndEquivalents: fund.cashAndEquivalents,
      totalDebt: fund.totalDebt, debtToEquity: fund.debtToEquity,
      sharesOutstandingChange: fund.sharesOutstandingChange,
      dividendYield, grossMarginCurrent: fund.grossMarginCurrent,
      capex: fund.capex, epsGaap: fund.epsDiluted, sectorAvgPE, sector,
      institutionalData, macroContext: macroContext,
      beta: enriched.beta, forwardPE: enriched.forwardPE,
      operatingMargin: fund.operatingMargin, sainConsensus,
    });
  }
  ...
  try {
    const valInput = {
      ticker, currentPrice, sector: sector || fund.sector,
      marketCap: fund.marketCap, beta: enriched.beta,
      debtToEquity: fund.debtToEquity, totalDebt: fund.totalDebt,
      cashAndEquivalents: fund.cashAndEquivalents,
      freeCashFlow: fund.freeCashFlow, ttmFCF: fund.freeCashFlow,
      revenueCurrent: fund.revenueCurrent, ttmRevenue: fund.revenueCurrent,
      revenueGrowth3YrAvg: fund.revenueGrowth3YrAvg,
      fcfGrowth3YrAvg: fund.fcfGrowthYoY, fcfMargin: fund.fcfMargin,
      dilutedShares: fund.dilutedShares, sharesOutstanding: fund.sharesOutstanding,
      ttmEBITDA: fund.ebitda, dividendYield,
      historicalEVSales5YrAvg: enriched.evSales5YrAvg,
      historicalEVEBITDA5YrAvg: enriched.evEbitda5YrAvg,
    };
    const valuation = computeThreePillarValuation(valInput);
  ...
  try {
    const lynchClass = classifyLynch({
      epsGrowthYoY: fund.epsGrowthYoY || 0, epsGrowth5Yr: fund.epsGrowth5Yr || 0,
      netIncome: fund.netIncome || 0, netIncomePriorYear: fund.netIncomePriorYear || 0,
      revenueGrowthYoY: fund.revenueGrowthPct || 0, sector: sector || fund.sector,
    });
    row.lynch_category = lynchClass.category;
    row.lynch_hold_period = lynchClass.holdPeriod;
    row.lynch_sell_trigger = lynchClass.sellTrigger;
    if (fund.peRatio && (fund.epsGrowthYoY || fund.epsGrowth5Yr || 0) > 0) {
      row.peg_ratio = Math.round((fund.peRatio / (fund.epsGrowthYoY || fund.epsGrowth5Yr)) * 100) / 100;
    }
    const killThesis = checkKillThesis({
      debtToEquity: fund.debtToEquity || 0, fcfMargin: fund.fcfMargin || 0,
      gaapNonGaapDivergence: enriched.gaapDivergence || 0,
    });
    row.kill_thesis_flags = killThesis.flags.length > 0 ? killThesis.flags : null;
    row.kill_thesis_count = killThesis.killCount;
    ...
    if (row.avg_price_target && currentPrice) {
      const mos = computeMarginOfSafety(row.avg_price_target, currentPrice);
      if (mos) { row.margin_of_safety = mos.marginOfSafety; row.mos_recommendation = mos.recommendation; }
    }
    const ratingResult = assignRating(
      { revenueGrowthYoY: fund.revenueGrowthPct || 0, ebitdaGrowthYoY: fund.ebitdaGrowthYoY || 0,
        debtToEbitda: (fund.totalDebt && fund.ebitda && fund.ebitda > 0) ? fund.totalDebt / fund.ebitda : 0,
        sector: sector || fund.sector },
      { compositeUpside: row.avg_upside_pct || 0, totalReturn: row.total_return || (row.avg_upside_pct || 0) + (dividendYield || 0) },
      0, lynchClass.category
    );
    if (killThesis.forceDowngrade && ['STRONG_BUY', 'BUY'].includes(ratingResult.rating)) {
      ratingResult.rating = 'NEUTRAL';
    }
  ...
  if (upsertErr) {
    console.error(`[deepScoreSingle] Upsert error for ${ticker}:`, upsertErr.message);
    return { error: upsertErr.message, row };
  }

  console.log(`[deepScoreSingle] ${ticker}: score=${scores.totalScore} signal=${scores.signal} eps=${fund.epsDiluted} net_income=${fund.netIncome} fcf=${fund.freeCashFlow}`);
  ...
  return {
    ...
    signal: scores.signal,
    fundamentals: {
      eps_gaap: fund.epsDiluted, net_income: fund.netIncome,
      operating_income: fund.operatingIncome, operating_margin: fund.operatingMargin,
      free_cash_flow: fund.freeCashFlow, fcf_margin: fund.fcfMargin,
      ma_50d: ma50d, price_200wma: price200WMA, price_200mma: price200MMA,
    },
    valuation: {
      rating: row.valuation_rating, avg_target: row.avg_price_target,
      avg_upside: row.avg_upside_pct,
    },
  };
}

module.exports = { runDeepScore, deepScoreSingle };
